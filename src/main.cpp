...
// No parent at construction — M5.Lcd is a reference member whose target
// binding may not be initialized before globals' constructors run. Parent
// is passed explicitly at pushSprite() calls below (setup() and loop()).
M5Canvas spr;
...
const uint16_t PANEL = 0x4A49;   // overlay panel background — tuned for RGB332 (quantizes to 0x49, a neutral dark grey)
...
void setup() {
  ...
  // Shrink speaker DMA buffers so I2S alloc succeeds on a tight heap —
  // defaults (len=256, count=8 → ~4KB DMA) fail once heap drops, and the
  // repeated retries on every beep() flood logs and trigger a WDT reset.
  // Happens now while heap is fresh (~150KB free).
  {
    auto spk_cfg = M5.Speaker.config();
    spk_cfg.dma_buf_len   = 128;
    spk_cfg.dma_buf_count = 3;
  ...
  // 8bpp RGB332 — halves sprite to ~32KB (vs 64KB at 16bpp) so I2S DMA
  // and BLE pairing buffers have room. Nearest-color conversion happens
  // on draw, so existing uint16_t color constants still work.
  spr.setColorDepth(8);
